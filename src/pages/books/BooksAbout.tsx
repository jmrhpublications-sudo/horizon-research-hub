import { memo } from "react";
import { Link } from "react-router-dom";
import Header from "@/components/layout/Header";
import Footer from "@/components/layout/Footer";
import SEOHead from "@/components/seo/SEOHead";

const BooksAbout = memo(() => {
  return (
    <div className="min-h-screen bg-background font-sans">
      <SEOHead 
        title="About Book Publishing | JMRH Publications"
        description="Learn about JMRH Publications' academic book publishing services for edited volumes, monographs, and conference proceedings."
        canonical="/books/about"
      />
      <Header />
      
      {/* Page Header */}
      <section className="pt-32 pb-16 bg-gradient-to-b from-oxford/5 to-transparent">
        <div className="container max-w-4xl mx-auto px-6">
          <nav className="flex items-center gap-2 text-xs uppercase tracking-widest text-oxford/40 mb-6">
            <Link to="/" className="hover:text-gold transition-colors">Home</Link>
            <span>/</span>
            <span className="text-gold">Books</span>
          </nav>
          <h1 className="font-serif text-4xl md:text-5xl font-black text-oxford mb-6">
            About Book Publishing
          </h1>
        </div>
      </section>

      {/* Content */}
      <section className="py-16">
        <div className="container max-w-4xl mx-auto px-6">
          <div className="bg-white border border-black/5 p-8 md:p-12">
            <p className="text-oxford/70 leading-relaxed mb-8">
              JMRH Publications offers structured academic book publishing services for edited volumes, scholarly monographs, co-authored books, and conference proceedings. All books are published with valid ISBN registration and proper bibliographic documentation, ensuring academic credibility and formal recognition.
            </p>

            {/* Publishing Focus */}
            <div className="mb-12">
              <h2 className="font-serif text-2xl font-bold text-oxford mb-6 pb-4 border-b border-gold/20">What We Publish</h2>
              <div className="grid md:grid-cols-2 gap-4">
                {[
                  "Edited Academic Volumes",
                  "Single Author Research Books",
                  "Co-authored Books",
                  "Conference Proceedings",
                  "Thematic Research Collections"
                ].map((item, index) => (
                  <div key={index} className="flex items-start gap-3 p-4 bg-oxford/5">
                    <span className="w-2 h-2 rounded-full bg-gold mt-2 flex-shrink-0" />
                    <span className="text-oxford/80">{item}</span>
                  </div>
                ))}
              </div>
            </div>

            {/* Publishing Process */}
            <div>
              <h2 className="font-serif text-2xl font-bold text-oxford mb-6 pb-4 border-b border-gold/20">Our Publishing Process Emphasizes</h2>
              <div className="grid md:grid-cols-2 gap-4">
                {[
                  "Academic quality and structured review",
                  "Ethical publishing practices",
                  "Proper ISBN registration",
                  "Professional formatting and documentation",
                  "Transparent editorial procedures"
                ].map((item, index) => (
                  <div key={index} className="flex items-center gap-3 p-4 bg-gold/5 border border-gold/10">
                    <span className="w-6 h-6 rounded-full bg-gold text-white flex items-center justify-center text-sm font-bold">✓</span>
                    <span className="text-oxford/80">{item}</span>
                  </div>
                ))}
              </div>
            </div>

            <div className="mt-12 flex flex-wrap gap-4">
              <Link 
                to="/books/proposal"
                className="inline-flex items-center gap-2 bg-oxford text-white px-6 py-3 text-xs uppercase tracking-widest font-bold hover:bg-gold hover:text-oxford transition-all"
              >
                Submit Book Proposal
              </Link>
              <Link 
                to="/books/published"
                className="inline-flex items-center gap-2 border border-oxford text-oxford px-6 py-3 text-xs uppercase tracking-widest font-bold hover:bg-oxford hover:text-white transition-all"
              >
                View Published Books
              </Link>
            </div>
          </div>
        </div>
      </section>

      <Footer />
    </div>
  );
});

export default BooksAbout;
